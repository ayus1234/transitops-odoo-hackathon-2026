...
def random_date_past(days_back=365):
    return datetime.now(timezone.utc) - timedelta(days=random.randint(0, days_back), hours=random.randint(0, 23), minutes=random.randint(0, 59))

def random_date_future(days_forward=180):
    return datetime.now(timezone.utc) + timedelta(days=random.randint(1, days_forward), hours=random.randint(0, 23), minutes=random.randint(0, 59))
...
def run():
    ...
    maintenances = []
    technicians = ["Mike R.", "Sarah K.", "Alex M.", "David J.", "Jennifer L."]
    for i in range(NUM_MAINTENANCE):
        status = random.choice(['Completed', 'Pending', 'In Progress', 'Approved', 'Rejected'])
        assigned_tech = random.choice(technicians) if status != 'Pending' else None
        maintenances.append(Maintenance(
            ...
            scheduled_date=random_date_past(180).date() if status == 'Completed' else random_date_future(30).date(),
