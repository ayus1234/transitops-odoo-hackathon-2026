...
def get_upcoming_jobs(
    ...
    service = MaintenanceService(db)
    events = service.get_scheduler_events(current_user=current_user)
    upcoming = [e for e in events if e.status in ['Approved', 'In Progress'] and e.scheduled_date >= date.today()]
...
def create_maintenance(
    ...
    """
    service = MaintenanceService(db)
    record = service.create_maintenance(data, created_by=current_user.id)  # type: ignore
