    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-background">
      ...
      <div className="p-3 md:p-lg space-y-lg flex-1 overflow-y-auto custom-scrollbar min-w-0">
                    ...
                    {loading ? (
                      ...
                      <tr key={record.id} className="hover:bg-surface-container-low transition-colors group">
                        ...
                        <td className="px-md py-3 flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${record.assigned_technician ? 'bg-primary/20 text-primary' : 'bg-outline-variant text-on-surface-variant'}`}>
                            {getInitials(record.assigned_technician)}
                          </div>
                          {record.assigned_technician ? (
                            <span className="text-body-sm truncate max-w-[100px]">{record.assigned_technician}</span>
                          ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-md pb-lg">
                ...
                technicianWorkload.map(tech => (
                  <div key={tech.name} className="space-y-1">
                    <div className="flex justify-between text-body-sm font-bold">
                      <span>{tech.name}</span>
                      <span>{tech.count}/{tech.maxCapacity} Tasks</span>
                    </div>
          ...
          {/* Inventory Alert Card (Static as per HTML since no backend schema exists) */}
          <div className="bg-tertiary-container/10 p-md rounded-lg border border-tertiary/20 shadow-sm col-span-1 flex flex-col justify-between min-h-[220px]">
            <div>
              ...
              <p className="text-body-sm text-on-tertiary-fixed-variant">Critical shortage detected for <span className="font-bold">Heavy Duty Brake Pads (Ref: BP-900)</span>. 3 vehicles waiting.</p>
            </div>
            <button 
                    ...
                    scheduledTimeline.map((item, idx) => (
                      <div key={item.id} className="flex gap-md">
                        <div className="relative z-10 w-12 flex flex-col items-center">
                          <div className={`bg-surface border-2 ${idx === 0 ? 'border-primary text-primary' : 'border-outline-variant text-on-surface-variant'} w-10 h-10 rounded-lg flex flex-col items-center justify-center shadow-sm`}>
                            <span className="text-[10px] font-bold uppercase leading-none mt-1">{item.month}</span>
                            <span className="text-body-md font-bold leading-none">{item.day}</span>
                          </div>
                        </div>
                        <div className="flex-1 pb-4">
                          <h4 className="text-body-md font-bold text-on-surface">{item.maintenance_type}</h4>
                          <p className="text-body-sm text-on-surface-variant">Vehicle: <span className="font-data-tabular text-on-surface">{item.vehicle?.registration_number}</span></p>
