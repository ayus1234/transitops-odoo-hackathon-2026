...
export const MapBoundsFitter = ({ bounds }) => {
  const map = useMap();
...
const FleetMapView = ({ isModal = false, onClose }) => {
  ...
  const stats = {
    ...
    activeVehicles: fleetData.vehicles.filter(v => ['active', 'on trip'].includes(v.status?.toLowerCase())).length,
    totalDrivers: fleetData.drivers.length,
    activeTrips: fleetData.trips.length,
    inMaintenance: fleetData.vehicles.filter(v => ['maintenance', 'in shop'].includes(v.status?.toLowerCase())).length,
    critical: fleetData.vehicles.filter(v => v.has_critical_alert).length,
  };
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className="flex-1 relative bg-slate-100">
        ...
        {(!mapLoading && fleetData.vehicles.length === 0 && fleetData.drivers.length === 0 && !fleetData.bounds?.min_lat) ? (
          ...
          >
            ...
            />
            <MapBoundsFitter bounds={fleetData.bounds} />
      ...
      {/* Footer Stats */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-0 border-t border-outline-variant bg-surface-container-lowest shrink-0">
        <div className="p-3 border-r border-b md:border-b-0 border-outline-variant text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">Total Vehicles</p>
          <p className="text-title-md font-bold text-on-surface">{stats.totalVehicles}</p>
        </div>
        <div className="p-3 border-r border-b md:border-b-0 border-outline-variant text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">Active Vehicles</p>
          <p className="text-title-md font-bold text-primary">{stats.activeVehicles}</p>
        </div>
        <div className="p-3 border-r border-b md:border-b-0 border-outline-variant text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">Total Drivers</p>
          <p className="text-title-md font-bold text-on-surface">{stats.totalDrivers}</p>
        </div>
        <div className="p-3 border-r border-b md:border-b-0 border-outline-variant text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">Active Trips</p>
          <p className="text-title-md font-bold text-blue-600">{stats.activeTrips}</p>
        </div>
        <div className="p-3 border-r border-outline-variant text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">In Maintenance</p>
          <p className="text-title-md font-bold text-orange-600">{stats.inMaintenance}</p>
        </div>
        <div className="p-3 text-center">
          <p className="text-[11px] text-on-surface-variant font-medium uppercase tracking-wider">Critical Alerts</p>
          <p className={`text-title-md font-bold ${stats.critical > 0 ? 'text-error' : 'text-on-surface'}`}>{stats.critical}</p>
        </div>
      </div>
    </div>
  );
